    ...
    def process(self, record):
        ...
        for notification in self.notifications:
            if notification.enabled and record.get('state') not in ['ack', 'close'] and notification.match(record):
                proclog.info("Notification %s matched record", notification.name, extra=context)
...
class NotificationObject:
    ...
    def __init__(self, notification, plugin):
        context = dict(plugin='notification', uid=notification.get('uid'))
        ...
        if isinstance(self.actions, list) and len(self.actions) > 0:
            if self.actions:
                ...
                apilog.warning("No action defined in '%s', disabling notification", self.name, extra=context)
                self.enabled = False
        elif self.enabled:
            apilog.warning("No action defined in '%s', disabling notification", self.name, extra=context)
            self.enabled = False
        apilog.debug("Initialized with action plugins: %s", [str(a) for a in self.action_plugins], extra=context)
        # Initializing the time constraints
        apilog.debug("Init Notification filter %s Time Constraints", self.name, extra=context)
    ...
    def send(self, record):
        ...
        context = dict(plugin='notification', rid=record.get('uid'))
        ...
        if len(self.action_plugins) > 0:
            ...
            for action_plugin in self.action_plugins:
                proclog.info("Triggering action '%s'", action_plugin.name, extra=context)
                action_plugin.send(action_obj)
        else:
            proclog.error("Notification %s has no action. Cannot send", self.name, extra=context)
