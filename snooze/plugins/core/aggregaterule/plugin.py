...
class Aggregaterule(Plugin):
    ...
    def process(self, record: dict):
        ...
        context = dict(plugin=self.name, rid=record.get('uid'))
        ...
        for aggrule in self.aggregate_rules:
            if aggrule.enabled and aggrule.match(record):
                ...
                proclog.info("Computed hash %s (from '%s')", record['hash'], self.name, extra=context)
                record = self.match_aggregate(record, aggrule.throttle, aggrule.flapping, aggrule.watch, aggrule.name)
                break
        else:
            proclog.debug('No aggregate rule matched, will use the default aggregate rule', extra=context)
            if 'raw' in record:
            ...
            proclog.info("Computed hash %s (from default aggregate rule)", record['hash'], extra=context)
    ...
    def match_aggregate(self, record, throttle=10, flapping=3, watch=[], aggrule_name='default'):
        ...
        context = dict(plugin=self.name, rid=str(record.get('uid')))
        proclog.debug("Checking in the db for hash %s", record['hash'], extra=context)
        aggregate = self.db.get_one('record', dict(hash=record['hash']))
        if aggregate:
            proclog.debug("Found hash %s in db", record['hash'], extra=context)
            now = datetime.datetime.now()
            record = dict(list(aggregate.items()) + list(record.items()))
            record_state = record.get('state', '')
            proclog.info("UID change to %s", aggregate['uid'], extra=context)
            ...
            if record_state == 'close':
                ...
                if record.get('state') != 'close':
                    proclog.info("OK received, closing alert", extra=context)
                    ...
                    proclog.info("OK received but the alert is already closed, discarding", extra=context)
            ...
            for watched_field in watch:
                ...
                if record_field != aggregate_field:
                    proclog.info("The watch field '%s' changed: %s -> %s", watched_field, aggregate_field, record_field, extra=context)
            ...
            if watched_fields:
                ...
                proclog.debug("Issued '%s' comment because of watch field change", comment['type'], extra=context)
                record['flapping_countdown'] = aggregate.get('flapping_countdown', flapping) - 1
            elif record.get('state') == 'close':
                proclog.info("Auto-reopening the aggregate %s", record['hash'], extra=context)
                comment['message'] = 'Auto re-opened'
                comment['type'] = 'open'
                record['state'] = 'open'
                record['flapping_countdown'] = aggregate.get('flapping_countdown', flapping) - 1
            elif throttling:
                proclog.info("Alert throttled (time within %s range), discarding", throttle, extra=context)
            ...
            if flapping_count == 0:
                ...
                proclog.info("Flapping detected. Stopped notification until throlle expires (%d sec left)", seconds_left, extra=context)
            ...
            proclog.info("Creating aggregate %s", record['hash'], extra=context)
            matched = self.db.replace_one('aggregate', {'hash': record['hash']}, record, update_time=False)
            if matched > 0:
                proclog.warning("Received 2 alerts with same hash %s at the same time. Discarding one", record['hash'], extra=context)
                raise Abort()
            record['duplicates'] = 1
        record.pop('snoozed', '')
...
class AggregateruleObject:
    def __init__(self, aggregate_rule):
        ...
        context = dict(plugin='aggregaterule', uid=aggregate_rule.get('uid'))
        apilog.debug("Instantiating aggregate rule: %s", self.name, extra=context)
