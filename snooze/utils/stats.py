    ...
    def init(self, metric: str, mtype: str, name: str, description: str, labels: List[str]):
        ...
        if self.enabled:
            if mtype == 'summary':
                ...
                for label in labels:
                    self.counters[metric][label] = AsyncIncrement(self.database, 'stats', ['date', label], '')
                    self.core.threads['asyncdb'].new_increment(self.counters[metric][label])
            else:
    ...
    def inc(self, metric_name, labels, amount=1):
        ...
        if self.enabled and metric_name in self.metrics:
            ...
            for key, value in labels.items():
                search = {'date': now, key: value}
                self.counters[metric_name][key].increment(search, amount)
