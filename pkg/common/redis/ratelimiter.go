...
/*
func Ratelimit(ctx context.Context, key string, burst, rate int, period time.Duration) (bool, error) {
	args := []interface{}{key, burst, rate, period, 1}
	v, err := ratelimitScript.Run(ctx, Client, []string{key}, args).Result()
	if err != nil {
		return false, err
	}
	values := v.([]interface{})
