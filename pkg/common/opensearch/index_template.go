...
func createIndexTemplate(ctx context.Context, name string, tpl models.IndexTemplate) error {
	log.Debugf("Creating index template '%s'...", name)

	body, err := json.Marshal(tpl)
	if err != nil {
		return fmt.Errorf("error marshaling index template '%s': %s", name, err)
	}
...
func hasIndexTemplate(ctx context.Context, name string) (bool, int, error) {
